...
dpgwtb = pd.DataFrame(
    data={"value": tabel["dpgwtb"].to_numpy()}, index=np.arange(nxlig - 1, nxuig + 2, 1)
)
...
def gwl_to_index(gwl: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    dpgw = mv - gwl
    igk = igdc["index_gwtb"][
        ((dpgw + dc) / ddgwtb).astype(dtype=np.int32)
    ].to_numpy()  # ddgwtb = stepsize in igdc array
    # ponding
    ponding = dpgw < 0.0
    igk[ponding] = igk[ponding] - 1
    # maximize on array size?
    igk = np.maximum(igk, nlig - 1)
    figk = (dpgw - dpgwtb["value"][igk].to_numpy()) / (
        dpgwtb["value"][igk + 1].to_numpy() - dpgwtb["value"][igk].to_numpy()
    )
    return igk, figk
...
head = mv - (
    dpgwtb["value"][ig].to_numpy()
    + (fig * (dpgwtb["value"][ig + 1].to_numpy() - dpgwtb["value"][ig].to_numpy()))
)
