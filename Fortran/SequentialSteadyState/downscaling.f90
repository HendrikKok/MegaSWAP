...
contains
    ...
    function t_databaseSet_readNCset(dbset, unsa_path) result (success)
        logical :: success
        ...
        allocate(dbset%dbs(NMAXRZ, NMAXSL))
        allocate(dbset%rzdepth(NMAXRZ))

        ! read table contents from files
        do sl=1, NMAXSL
           do rz=1, NMAXRZ
              write(dum,'(i3.3,a,i3.3)') sl, '_', rz
    ...
    function t_database_readNC(db,ncid) result (success)
        ...
        integer :: varid_qmr, varid_sv, varid_theta, varid_dpgw, varid_igdc
        integer :: varid_qmmat_min, varid_qmmat_plus, varid_prz
        integer :: varid_hbotb
        integer :: ios, iline
        integer :: lun, ir, ip, ig, sl, rz, sl_tgt, igdcmn, igdcmx
        integer :: ierr
        integer :: nxb, nuip, nlip, nuig, nlig, nxlig, nxuig, nxsl
        logical :: exists
...
!       success = realloc(db%thetatb,     1,     nxb, nxlig-1, nuig, nlip, nuip)
...
!       phi = ix_fix(sigma1d,sigma,lbound(sigma1d,dim=1),default=default)
    ...
    function t_downscaling_readNC(ds,ncid) result (success)
        ...
        character(len=3) dum
        integer :: dimid_node
        integer :: varid_theta, varid_dz
        integer :: ios, iline
        integer :: lun, ir, ip, ig, sl, rz, sl_tgt, igdcmn, igdcmx
        integer :: ierr, inode
        integer :: nnode, nuip, nlip, nuig, nlig, nxlig, nxuig
        logical :: exists
    
        success = .False.
        ierr = nf90_inq_dimid(ncid,'node',dimid_node)
        ierr = nf90_inquire_dimension(ncid, dimid_node, len=nnode)
        ierr = nf90_get_att(ncid, NF90_GLOBAL, 'igdcmn', igdcmn)
        ierr = nf90_get_att(ncid, NF90_GLOBAL, 'igdcmx', igdcmx)
        ...
        ! allocate arrays in db
        success = realloc(ds%thetatb, nxlig-1, nuig, nlip, nuip, 1, nnode)
        success = realloc(ds%z,  0, nnode)
        success = realloc(ds%dz, 1, nnode)
        ...
        ds%z(0)=0.d0
        do inode=1,nnode
           ds%z(inode)=ds%z(inode-1)-ds%dz(inode)
        enddo
    ...
    function t_downscalingSet_readNCset(dsset, unsa_path) result (success)
        logical :: success
